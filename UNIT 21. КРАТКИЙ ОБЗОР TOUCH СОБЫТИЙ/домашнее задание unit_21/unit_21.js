...
function t8(e) {
    randIndex = randNum(0, 5);
    e.target.style.background = `${a8[randIndex]}`;
}
...
function t10(e) {
    div10width++;
    document.querySelector('.div-10').style.width = `${div10width}px`;
}
...
function nextFunction() {
    let big_img = document.querySelector('.img-12-max');
    if(count ==5){
        images[count].classList.remove('active-img');
        count = 0;
        images[count].classList.add('active-img');
        big_img.setAttribute('src', `img/${count + 1}.png`);
    }else{
        images[count].classList.remove('active-img');
        count++;
        images[count].classList.add('active-img');
